...
export default function Home() {
  ...
  const processFile = async (f: File) => {
    // Jangan proses file baru kalau lagi analisis
    if (loading) return;
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    e.preventDefault();
    setDragging(false);
    if (loading) return; // Drop diblokir saat analisis
  ...
  const renderUploadForm = () => (
    <>
      <div
        ...
        className={`rounded-2xl border-2 border-dashed transition-all mb-4 p-7 ${dragging
            ? "border-teal-500 bg-teal-50"
            : file
              ? "border-teal-400 bg-teal-50/50"
              : loading
                ? "border-[#bcc9c6] bg-white/70 cursor-not-allowed opacity-60"
      ...
      >
        {fileLoading ? (
          ...
          <div className="relative flex items-center justify-between">
            {/* Overlay loading saat analisis berjalan */}
            {loading && (
              <div className="absolute inset-0 -m-3 rounded-xl bg-teal-50/90 backdrop-blur-sm flex items-center gap-3 px-4 z-10">
                <div className="w-5 h-5 rounded-full border-2 border-teal-200 border-t-teal-600 animate-spin flex-shrink-0" />
                <div>
                  <p className="text-xs font-semibold text-teal-700">Sedang menganalisis...</p>
                  <p className="text-[10px] text-teal-500 mt-0.5">Jangan tutup halaman ini</p>
                </div>
              </div>
            )}
            ...
            <button
              ...
              disabled={loading}
              className={`text-xs px-3 py-1.5 rounded-lg transition-colors ${loading
      ...
      {!isPDF && (
        <>
          ...
          <div className={`rounded-2xl border border-[#bcc9c6] bg-white/80 overflow-hidden mb-4 transition-opacity ${loading ? "opacity-50 pointer-events-none" : ""}`}>
            <textarea
              value={text}
              onChange={(e) => setText(e.target.value)}
              placeholder="Paste teks di sini..."
              disabled={loading}
              className="w-full px-5 pt-5 pb-3 resize-none outline-none text-sm leading-relaxed bg-transparent text-[#0b1c30] placeholder:text-[#6d7a77] disabled:cursor-not-allowed"
  ...
  // ── FULL LANDING (user baru, belum punya history) ────────────────────
        ...
        {sidebarOpen && (
          <aside className="w-60 flex-shrink-0 border-r border-[#e5eeff] bg-white/70 flex flex-col overflow-hidden">
            <div className="px-4 py-4 border-b border-[#e5eeff]">
              <p className="text-[10px] font-bold text-[#6d7a77] uppercase tracking-widest mb-3">
                Scan History
              </p>
              <button
                onClick={reset}
                disabled={loading}
                className={`w-full flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-semibold transition-colors ${loading
            ...
            <div className="px-3 py-2">
              <p className="text-[10px] font-semibold text-[#6d7a77] uppercase tracking-widest px-1 mb-1">
                Last 30 Days
              </p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-2 pb-4 space-y-0.5">
              {history.map((record) => (
                <button
                  key={record.id}
                  onClick={() => {
                    if (loading) return; // Blokir ganti history saat analisis
                    setActiveScan(record);
                    setActiveHighlight(null);
                    setStage("result");
                  }}
                  className={`w-full text-left px-3 py-2.5 rounded-xl transition-colors ${loading
                      ? "opacity-50 cursor-not-allowed"
                      : activeScan?.id === record.id && stage === "result"
                        ? "bg-teal-50 border border-teal-200"
                        : "hover:bg-[#f0fdfa]"
                    }`}
                >
                  <p
                    className={`text-xs font-medium truncate mb-0.5 ${activeScan?.id === record.id && stage === "result"
                        ? "text-teal-700"
                        : "text-[#0b1c30]"
                      }`}
                  >
                  ...
                  <div className="flex items-center gap-2">
                    {record.integrityScore !== null && (
                      <span
                        className={`text-xs font-bold ${record.integrityScore >= 75
                            ? "text-teal-600"
                            : record.integrityScore >= 50
                              ? "text-amber-500"
                              : "text-red-500"
                          }`}
                      >
                    ...
                    <span className="text-xs text-[#6d7a77]">
                      {record.timestamp.toLocaleTimeString("id", {
                        hour: "2-digit",
                        minute: "2-digit",
                      })}
                    </span>
